...
class State:
    def __init__(self, position=None, velocity=None, heading=None, history_size=20):
        ...
        # Realistic drone constraints (based on DJI drones)
        self.max_horizontal_speed = 20.0  # m/s (typical DJI max speed ~20 m/s)
        self.max_vertical_speed = 5.0     # m/s (typical ascent/descent speed)
        self.max_horizontal_accel = 8.0   # m/s^2 (realistic acceleration)
        self.max_vertical_accel = 4.0     # m/s^2 (vertical acceleration)

    def apply_realistic_constraints(self, desired_velocity, dt):
        """
        Apply realistic DJI drone speed and acceleration constraints.

        Args:
            desired_velocity: [vx, vy, vz] desired velocity
            dt: time step

        Returns:
            constrained_velocity: velocity after applying constraints
        """
        import numpy as np

        desired_vel = np.array(desired_velocity)
        current_vel = np.array(self.velocity)

        # Calculate desired acceleration
        desired_accel = (desired_vel - current_vel) / dt

        # Limit horizontal acceleration (x, y)
        horizontal_accel = desired_accel[:2]
        horizontal_accel_mag = np.linalg.norm(horizontal_accel)
        if horizontal_accel_mag > self.max_horizontal_accel:
            horizontal_accel = horizontal_accel * (self.max_horizontal_accel / horizontal_accel_mag)

        # Limit vertical acceleration (z)
        vertical_accel = np.clip(desired_accel[2], -self.max_vertical_accel, self.max_vertical_accel)

        # Apply limited acceleration
        constrained_accel = np.array([horizontal_accel[0], horizontal_accel[1], vertical_accel])
        constrained_velocity = current_vel + constrained_accel * dt

        # Limit horizontal speed
        horizontal_vel = constrained_velocity[:2]
        horizontal_speed = np.linalg.norm(horizontal_vel)
        if horizontal_speed > self.max_horizontal_speed:
            horizontal_vel = horizontal_vel * (self.max_horizontal_speed / horizontal_speed)
            constrained_velocity[:2] = horizontal_vel

        # Limit vertical speed
        constrained_velocity[2] = np.clip(constrained_velocity[2],
                                          -self.max_vertical_speed,
                                          self.max_vertical_speed)

        return constrained_velocity.tolist()
...
class AdaptiveController:
    ...
    def __init__(self, adaptation_gain=200.0, damping_gain=200.0, lambda_gain=10.0):
        ...
        # Controller gains
        self.P = adaptation_gain * np.eye(5)  # 5 parameters for 3D drone model

        # Higher damping on z-axis for better altitude control
        self.Kd = np.diag([damping_gain, damping_gain, damping_gain * 1.5])
        self.lambda_gain = lambda_gain

        # Initialize parameters with reasonable guesses instead of zeros
        # Parameters: [mass, drag_x, drag_y, drag_z, m*g]
        self.ahat = np.array([1.0, 0.1, 0.1, 0.1, 10.0])  # Better initial guess

        # Parameter bounds for projection
        self.param_min = np.array([0.5, 0.0, 0.0, 0.0, 5.0])    # Lower bounds
        self.param_max = np.array([5.0, 2.0, 2.0, 2.0, 50.0])   # Upper bounds
    ...
    def _build_regressor_matrix(self, q, qdot, qr_dot, qr_ddot):
        ...
        - m: mass
        - D: drag matrix
        - G: gravity vector [0, 0, -m*g]

        Linearly parameterized as: Y * a = required force
        Parameters a = [m, dx, dy, dz, m*g]
        """
        import numpy as np

        Y = np.zeros((3, 5))

        # Row 1: x-axis dynamics (no gravity)
        Y[0, 0] = qr_ddot[0]           # mass * acceleration
        Y[0, 1] = qdot[0]              # drag term proportional to velocity

        # Row 2: y-axis dynamics (no gravity)
        Y[1, 0] = qr_ddot[1]           # mass * acceleration
        Y[1, 2] = qdot[1]              # drag term proportional to velocity

        # Row 3: z-axis dynamics (includes gravity)
        Y[2, 0] = qr_ddot[2]           # mass * acceleration
        Y[2, 3] = qdot[2]              # drag term proportional to velocity
        Y[2, 4] = -1.0                 # gravity term (negative because gravity pulls down)

        return Y

    def update_parameters(self, ahat_dot, dt):
        """
        Update estimated parameters using adaptation law with projection.
        ...
        # Project parameters to stay within bounds (prevents drift)
        self.ahat = np.clip(self.ahat, self.param_min, self.param_max)
